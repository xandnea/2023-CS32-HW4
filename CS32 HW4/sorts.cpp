...
void insertion_sort(vector<Student>& s, bool comp(const Student&, const Student&))
{
    for (int unsorted = 1; unsorted < s.size(); unsorted++)
    {
        ...
        int loc = unsorted;
        while (loc > 0 && !comp(s[loc - 1], nextItem)) // if comp is true then s1 is less than s2
        {
            s[loc] = s[loc - 1];
            loc--;
        }
...
void sortUsingPtrs(vector<Student>& students, bool comp(const Student*, const Student*))
{
    ...
    sort(studentsPtr.begin(), studentsPtr.end(), compareStudentPtr);
    ...
    //        order.
    students.clear();
    for (int i = 0; i < studentsPtr.size(); i++)
    {
        students.push_back(*studentsPtr[i]);
    }
